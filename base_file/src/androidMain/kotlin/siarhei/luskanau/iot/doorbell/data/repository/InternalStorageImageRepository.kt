...
class InternalStorageImageRepository(private val context: Context) : ImageRepository {

    override fun prepareFile(name: String): File =
        File.createTempFile("camera_${name}_", null, context.cacheDir)

    override fun saveImage(byteBuffer: ByteBuffer?, name: String): ImageFile = runCatching {
    ...
    override fun saveImage(file: File): ImageFile = runCatching {
        ImageFile(
            name = file.name,
            path = file.absolutePath,
            size = file.length()
